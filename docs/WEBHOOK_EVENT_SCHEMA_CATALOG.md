...
> of the integration guide). Only transaction-level activity is currently
> surfaced through the HTTP webhook pipeline described here. If you need
> the full contract event set, consume it from Soroban RPC directly.
...
## Payload shape (current version, both event types)
...
Both event types share this exact payload shape today. They're kept as
